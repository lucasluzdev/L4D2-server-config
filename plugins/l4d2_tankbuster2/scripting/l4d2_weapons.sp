...
new Handle:g_max_give[47];
new max_give[47]; //array for storing initial quota of each item
...
public OnPluginStart()
{
	...
	g_max_give[29] = CreateConVar("sm_quota_molovtov", "-1", " Quota Given to each player for obtaining molovtov in each round ( -1 = unlimited 0 = disabled )");
		...
		switch (itemNum)
		{
			...
			case 29: // molovtov
			{
				if ( give_quota29[client] > 0 || give_quota29[client] < 0) {
					//Give the player a molovtov
					FakeClientCommand(client, "give molovtov");
					//Decrease remaining quota of that player by 1
					give_quota29[client]--;
					//Notify remaining quota
					if (give_quota29[client] > 0) {
						PrintToChat(client, "\x04[SM] \x01You have %d more chance to obtain a molovtov until next round",give_quota29[client]);
					}
					else if (give_quota29[client] == 0){
						PrintToChat(client, "\x04[SM] \x01You cannot obtain anymore molovtov until next round");
					}
				}
				else {
					//No more quota left
					PrintToChat(client, "\x04[SM] \x01You cannot obtain a molovtov");
				}
			}
			case 30: // oxygentank
			{
