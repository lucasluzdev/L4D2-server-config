/*===========================================================================================================
	...
	General Updates:
 *  11-09-2024 > Version 2.8: General enhancements, fixing a bug and adding russian translation, thanks to JustMadMan.
 *  10-09-2024 > Version 2.7: Fixing translations, thanks to HarryPotter.
 *  09-09-2024 > Version 2.6: Fixing sometimes players healing inappropriately specially when using other plugins like heartbeats.
...
public void OnPluginStart()
{
	...
	Cvar_DoorLock_AddCheats = CreateConVar("l4d2_doorlock_add_cheats", "1", "Set Cheats While Safe Area Locked (0 = No Cheats, 1 = No DMG, 2 = Infine Ammo, 3 = All)", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	Cvar_DoorLock_Countdown = CreateConVar("l4d2_doorlock_countdown", "20", "How Long You Want To Set The Countdown To Unlock The Safe Area (In Seconds)", FCVAR_NOTIFY);
	...
	Cvar_DoorLock_LeaverMsg = CreateConVar("l4d2_doorlock_leavers_notify", "1", "Display Chat Texts To Leavers When Teleported (0 = Disable, 1 = Chat, 2 = Central Text)", FCVAR_NOTIFY, true, 0.0, true, 2.0);
...
{
	if(!g_bLockSafeAreas && g_bLeftSafeAreas) //Force round to start only when someone leaves and saferoom is unlock
	{
...
{
	if(!g_bLeftSafeAreas && !GetRealTeamCount(2) && !GetRealTeamCount(3))
	{
		g_iUnlocksTime = Cvar_DoorLock_Countdown.IntValue;
		return Plugin_Continue;
	}
...
Action Command_Lock(int client, int args)
{
	...
	else if(g_bLeftSafeAreas) CPrintToChat(client, "%T", "Round Started", client);
	else if(g_bLockSafeAreas) CPrintToChat(client, "%T", "Saferoom Locked", client);
	else if(g_iGiveUpsTime <= 0) CPrintToChat(client, "%T", "Ready Up Time Ended", client);
	else
	{
...
{
	if (!g_bIsGameAllowed) return Plugin_Handled;
	else if(!g_bLockSafeAreas) CPrintToChat(client, "%T", "Saferoom Unlocked", client);
	else
	{
		g_bAdminTakeover = true;
		PrintHintTextToAll("%t", "Move Out", client);
		UnFreezePlayersInFirstChapters();
		UnLockAllRotatingSaferoomDoors();
		UnTriggerSafeAreaLocksFeatures();
		CPrintToChatAll("%t", "Admin Unlocks", client, client);
	}
	return Plugin_Handled;
}
...
Action Command_Ready(int client, int args)
{
	...
	else if(GetClientTeam(client) == 1) CPrintToChat(client, "%T", "Spectator Ready", client);
	else if(g_bLeftSafeAreas) CPrintToChat(client, "%T", "Round Started", client);
	else if(g_bClientIsReady[client])CPrintToChat(client, "%T", "You Are Ready", client);
	else if(g_iGiveUpsTime <= 0) CPrintToChat(client, "%T", "Ready Up Time Ended", client);
	else if(g_bAdminTakeover) CPrintToChat(client, "%T", "Admin Controls", client);
	else
	{
		g_bClientIsReady[client] = true;
		CPrintToChat(client, "%T", "Player Is Ready", client);
...
{
	if(!g_bIsGameAllowed || !Cvar_DoorLock_EnableRdy.BoolValue) return Plugin_Handled;
	else if(client == 0) PrintToServer("This command can not be executed by server", client);
	else if(GetClientTeam(client) == 1) CPrintToChat(client, "%T", "Spectator Ready", client);
	else if(g_bLeftSafeAreas) CPrintToChat(client, "%T", "Round Started", client);
	else if(!g_bClientIsReady[client]) CPrintToChat(client, "%T", "You Are Unready", client);
	else if(g_iUnrdyCounts[client] < 1) CPrintToChat(client, "%T", "Ready Up Limit", client);
	else if(g_iGiveUpsTime <= 0) CPrintToChat(client, "%T", "Ready Up Time Ended", client);
	else if(g_bAdminTakeover) CPrintToChat(client, "%T", "Admin Controls", client);
	else
	{
...
void Create_ShowTeamsReadyStatus(int client)
{
	...
	FormatEx(sTemp, sizeof(sTemp), "%t", "Menu Title", client);
	panel.DrawItem(sTemp, ITEMDRAW_RAWLINE);
	
	panel.DrawItem(" ", ITEMDRAW_RAWLINE); // Spacer
	
	FormatEx(sTemp, sizeof(sTemp), "%t", "Menu Ready", client);
	panel.DrawItem(sTemp, !g_bClientIsReady[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	
	FormatEx(sTemp, sizeof(sTemp), "%t", "Menu Unready", client);
	panel.DrawItem(sTemp, g_bClientIsReady[client] ? ITEMDRAW_DEFAULT : ITEMDRAW_DISABLED);
	
	panel.DrawItem(" ", ITEMDRAW_RAWLINE); // Spacer
	
	if (GetRealTeamCount(2) > 0) FormatEx(sTemp, sizeof(sTemp), "%s %t: [%.1f%%]", TeamReadyUpPercentageReached(2) ? "▣" : "▢", "Menu Survivor", TeamReadyUpPercentage(2), client);
	else FormatEx(sTemp, sizeof(sTemp), "▣ %t: [100.0%%]", "Menu Survivor", client);
	
	panel.DrawItem(sTemp, ITEMDRAW_RAWLINE);
	
	if (GetRealTeamCount(3) > 0) FormatEx(sTemp, sizeof(sTemp), "%s %t: [%.1f%%]", TeamReadyUpPercentageReached(3) ? "▣" : "▢", "Menu Infected", TeamReadyUpPercentage(3), client);
	else FormatEx(sTemp, sizeof(sTemp), "▣ %t: [100.0%%]", "Menu Infected", client);
	
	panel.DrawItem(sTemp, ITEMDRAW_RAWLINE);
	
	panel.DrawItem(" ", ITEMDRAW_RAWLINE); // Spacer
	
	FormatEx(sTemp, sizeof(sTemp), "%t", "Menu Close", client);
...
{
	if(!CanTeleport(client)) return;
	
	if(!BlockClientRepetition(client))
	{
		if(Cvar_DoorLock_LeaverMsg.IntValue == 1) CPrintToChat(client, "%T", "Leavers Message", client);
		else if(Cvar_DoorLock_LeaverMsg.IntValue == 2) PrintCenterText(client, "%T", "Leavers Message", client);
	}
...
void HealSurvivor(int client)
{
	...
	if (!IsPlayerAlive(client) || IsPlayerIncapacitated(client) || IsHandingFromLedge(client))
...
bool CanTeleport(int client)
{
	if(!g_bIsGameAllowed || !g_bFirstScenario || !g_bLockSafeAreas || g_bLeftSafeAreas || !Cvar_DoorLock_ModesType.BoolValue || GetClientTeam(client) != 2) return false;
	return true;
}
