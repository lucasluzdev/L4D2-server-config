...
{
    name        = "Fun Votes",
    author      = "AlliedModders LLC, Vann09",
...
ConVar g_Cvar_FF_Easy;
ConVar g_Cvar_FF_Normal;
ConVar g_Cvar_FF_Hard;
ConVar g_Cvar_FF_Expert;

float  g_fFFEasyDefault;
float  g_fFFNormalDefault;
float  g_fFFHardDefault;
float  g_fFFExpertDefault;
...
void SetFriendlyFireEnabled(bool enable)
{
    if (g_Cvar_FF != null)
    {
        g_Cvar_FF.BoolValue = enable;
    }

    if (enable)
    {
        if (g_Cvar_FF_Easy != null)
        {
            g_Cvar_FF_Easy.FloatValue = g_fFFEasyDefault;
        }

        if (g_Cvar_FF_Normal != null)
        {
            g_Cvar_FF_Normal.FloatValue = g_fFFNormalDefault;
        }

        if (g_Cvar_FF_Hard != null)
        {
            g_Cvar_FF_Hard.FloatValue = g_fFFHardDefault;
        }

        if (g_Cvar_FF_Expert != null)
        {
            g_Cvar_FF_Expert.FloatValue = g_fFFExpertDefault;
        }
    }
    else
    {
        if (g_Cvar_FF_Easy != null)
        {
            g_Cvar_FF_Easy.FloatValue = 0.0;
        }

        if (g_Cvar_FF_Normal != null)
        {
            g_Cvar_FF_Normal.FloatValue = 0.0;
        }

        if (g_Cvar_FF_Hard != null)
        {
            g_Cvar_FF_Hard.FloatValue = 0.0;
        }

        if (g_Cvar_FF_Expert != null)
        {
            g_Cvar_FF_Expert.FloatValue = 0.0;
        }
    }
}
...
public void OnPluginStart()
{
    ...
    g_Cvar_FF_Easy   = FindConVar("survivor_friendly_fire_factor_easy");
    g_Cvar_FF_Normal = FindConVar("survivor_friendly_fire_factor_normal");
    g_Cvar_FF_Hard   = FindConVar("survivor_friendly_fire_factor_hard");
    g_Cvar_FF_Expert = FindConVar("survivor_friendly_fire_factor_expert");

    if (g_Cvar_FF_Easy != null)
    {
        g_fFFEasyDefault = g_Cvar_FF_Easy.FloatValue;
    }

    if (g_Cvar_FF_Normal != null)
    {
        g_fFFNormalDefault = g_Cvar_FF_Normal.FloatValue;
    }

    if (g_Cvar_FF_Hard != null)
    {
        g_fFFHardDefault = g_Cvar_FF_Hard.FloatValue;
    }

    if (g_Cvar_FF_Expert != null)
    {
        g_fFFExpertDefault = g_Cvar_FF_Expert.FloatValue;
    }
    ...
    else if (action == MenuAction_VoteEnd)
    {
        ...
        else
        {
            ...
            switch (g_voteType)
            {
                ...
                case (ff):
                {
                    bool enableFF = !g_Cvar_FF.BoolValue;
                    ...
                    LogAction(
                        -1,
                        -1,
                        "Changing friendly fire to %s due to vote.",
                        (enableFF ? "1" : "0"));

                    SetFriendlyFireEnabled(enableFF);
                    ...
                    if (enableFF)
                    {
                        LogAction(
                            -1,
                            -1,
                            "Restored FF factors: easy=%.2f normal=%.2f hard=%.2f expert=%.2f",
                            g_fFFEasyDefault,
                            g_fFFNormalDefault,
                            g_fFFHardDefault,
                            g_fFFExpertDefault);
                    ...
                    else
                    {
                        LogAction(
                            -1,
                            -1,
                            "Disabled friendly fire and set all survivor_friendly_fire_factor_* values to 0");
                    }
                }
            }
        }
